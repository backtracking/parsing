...
namespace Java.EditorExtensions
{
    ...
    public sealed partial class Tokenizer {
        private readonly JavaLanguageVersion _langVersion;
        ...
        public Tokenizer(JavaLanguageVersion version, ErrorSink errorSink = null, TokenizerOptions options = TokenizerOptions.None) {
        ...
        private Token Next() {
            ...
            if (_state.IncompleteComment != null && Peek() != EOF) {
                IncompleteComment prev = _state.IncompleteComment;
                _state.IncompleteComment = null;
            ...
            while (true) {
                switch (ch) {
                    ...
                    //case 'B':
                    //    _state.LastNewLine = false;
                    //    if (_langVersion >= JavaLanguageVersion.V26) {
        ...
        //private Token ReadMultiLineComment(int ch, int startAdd)
        //{
        //    int end_add = 0;
        ...
        //            _state.IncompleteComment = new IncompleteComment(true, false, false, false);
        ...
        private Token ContinueComment()
        {
            ...
            for (; ; )
            {
                ...
                if (ch == EOF)
                {
                    ...
                    UnexpectedEndOfComment(true);
                    string incompleteContents = GetTokenString();

                    _state.IncompleteComment = new IncompleteComment(true, true, false, false);
                    return new CommentToken(incompleteContents);
                }
                else if (ch == '*' && NextChar() == '/')
                {
                    break;
        ...
        private Token ContinueString(char quote, bool isRaw, bool isUnicode, bool isBytes, bool isTriple, int startAdd) {
            ...
            for (; ; ) {
                ...
                if (ch == EOF) {
                    ...
                    _state.IncompleteComment = new IncompleteComment(quote == '\'', isRaw, isUnicode, isTriple);
                    return new IncompleteStringErrorToken("<eof> while reading string", incompleteContents);
                } else if (ch == quote) {
        ...
        private Token MakeStringToken(char quote, bool isRaw, bool isUnicode, bool isBytes, bool isTriple, int start, int length) {
         ...
         private void UnexpectedEndOfComment(bool isIncomplete) {
            string message = "EOF while scanning multiline comment";
        ...
        private Token ReadNumber(int start) {
            ...
            if (start == '0') {
                ...
                } else if (_langVersion >= JavaLanguageVersion.V16) {
        ...
        private Token ReadName() {
            ...
            // Generated with scripts found inside IronPython repository (https://github.com/IronLanguages/main/tree/ipy-2.7)
            // main / Languages / IronPython / Scripts
            //kwlist = [
            ...
            //]
            #region Generated Java Keyword Lookup
            ...
            if (ch == 'i')
            {
                ...
                if (ch == 'n')
                {
                    ...
                    if (ch == 't')
                    {
                        ...
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordIntToken;
                        }
                        if (NextChar() == 'e' && NextChar() == 'r' && NextChar() == 'f' && NextChar() == 'a' && NextChar() == 'c' && NextChar() == 'e' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordInterfaceToken;
                        }
                    }
                    else if (ch == 's')
                    {
                        if (NextChar() == 't' && NextChar() == 'a' && NextChar() == 'n' && NextChar() == 'c' && NextChar() == 'e' && NextChar() == 'o' && NextChar() == 'f' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordInstanceofToken;
                        }
                    }
                }
                else if (ch == 'm')
                {
                    ...
                    if (ch == 'p')
                    {
                        ...
                        if (ch == 'l')
                        {
                            if (NextChar() == 'e' && NextChar() == 'm' && NextChar() == 'e' && NextChar() == 'n' && NextChar() == 't' && NextChar() == 's' && !IsNamePart(Peek()))
                            {
                                MarkTokenEnd();
                                return Tokens.KeywordImplementsToken;
                            }
                        }
                        else if (ch == 'o')
                        {
                            if (NextChar() == 'r' && NextChar() == 't' && !IsNamePart(Peek()))
                            {
                                MarkTokenEnd();
            ...
            {
                if (NextChar() == 'h' && NextChar() == 'i' && NextChar() == 'l' && NextChar() == 'e' && !IsNamePart(Peek()))
                {
                    MarkTokenEnd();
            ...
            else if (ch == 'v')
            {
                ...
                if (ch == 'o')
                {
                    ...
                    if (ch == 'l')
                    {
                        if (NextChar() == 'a' && NextChar() == 't' && NextChar() == 'i' && NextChar() == 'l' && NextChar() == 'e' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordVolatileToken;
                        }
                    }
                    else if (ch == 'i')
                    {
                        ...
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordVoidToken;
                        }
                    }
                }
            }
            else if (ch == 't')
            {
                ...
                if (ch == 'r')
                {
                    ...
                    {
                        if (NextChar() == 'n' && NextChar() == 's' && NextChar() == 'i' && NextChar() == 'e' && NextChar() == 'n' && NextChar() == 't' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordTransientToken;
                        }
                    }
                }
                else if (ch == 'h')
                {
                    ...
                    if (ch == 'i')
                    {
                        if (NextChar() == 's' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordThisToken;
                        }
                    }
                    else if (ch == 'r')
                    {
                        if (NextChar() == 'o' && NextChar() == 'w' && NextChar() == 's' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordThrowsToken;
                        }
                    }
                }
            }
            else if (ch == 's')
            {
                ...
                if (ch == 'h')
                {
                    if (NextChar() == 'o' && NextChar() == 'r' && NextChar() == 't' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordShortToken;
                    }
                }
                else if (ch == 'w')
                {
                    if (NextChar() == 'i' && NextChar() == 't' && NextChar() == 'c' && NextChar() == 'h' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordSwitchToken;
                    }
                }
                else if (ch == 'u')
                {
                    if (NextChar() == 'p' && NextChar() == 'e' && NextChar() == 'r' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordSuperToken;
                    }
                }
                else if (ch == 't')
                {
                    ...
                    if (ch == 'a')
                    {
                        if (NextChar() == 't' && NextChar() == 'i' && NextChar() == 'c' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordStaticToken;
                        }
                    }
                    else if (ch == 'r')
                    {
                        if (NextChar() == 'i' && NextChar() == 'c' && NextChar() == 't' && NextChar() == 'f' && NextChar() == 'p' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordStrictfpToken;
                        }
                    }
                }
                else if (ch == 'y')
                {
                    if (NextChar() == 'n' && NextChar() == 'c' && NextChar() == 'h' && NextChar() == 'r' && NextChar() == 'o' && NextChar() == 'n' && NextChar() == 'i' && NextChar() == 'z' && NextChar() == 'e' && NextChar() == 'd' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordSynchronizedToken;
                    }
                }
            }
            else if (ch == 'r')
            {
                if (NextChar() == 'e' && NextChar() == 't' && NextChar() == 'u' && NextChar() == 'r' && NextChar() == 'n' && !IsNamePart(Peek()))
                {
                    MarkTokenEnd();
            ...
            else if (ch == 'p')
            {
                ...
                if (ch == 'u')
                {
                    if (NextChar() == 'b' && NextChar() == 'l' && NextChar() == 'i' && NextChar() == 'c' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordPublicToken;
                    }
                }
                else if (ch == 'a')
                {
                    if (NextChar() == 'c' && NextChar() == 'k' && NextChar() == 'a' && NextChar() == 'g' && NextChar() == 'e' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordPackageToken;
                    }
                }
                else if (ch == 'r')
                {
                    ...
                    if (ch == 'i')
                    {
                        if (NextChar() == 'v' && NextChar() == 'a' && NextChar() == 't' && NextChar() == 'e' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordPrivateToken;
                        }
                    }
                    else if (ch == 'o')
                    {
                        if (NextChar() == 't' && NextChar() == 'e' && NextChar() == 'c' && NextChar() == 't' && NextChar() == 'e' && NextChar() == 'd' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordProtectedToken;
                        }
                    }
                }
            }
            ...
            {
                if (NextChar() == 'o' && NextChar() == 't' && NextChar() == 'o' && !IsNamePart(Peek()))
                {
                    MarkTokenEnd();
                    return Tokens.KeywordGotoToken;
                }
            }
            else if (ch == 'f')
            {
                ...
                if (ch == 'l')
                {
                    if (NextChar() == 'o' && NextChar() == 'a' && NextChar() == 't' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordFloatToken;
                    }
                }
                else if (ch == 'i')
                {
            ...
            else if (ch == 'e')
            {
                ...
                if (ch == 'n')
                {
                    if (NextChar() == 'u' && NextChar() == 'm' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordEnumToken;
                    }
                }
                else if (ch == 'x')
                {
                    if (NextChar() == 't' && NextChar() == 'e' && NextChar() == 'n' && NextChar() == 'd' && NextChar() == 's' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordExtendsToken;
                    }
                }
                else if (ch == 'l')
                {
                    if (NextChar() == 's' && NextChar() == 'e' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
            ...
            else if (ch == 'd')
            {
                ...
                if (ch == 'e')
                {
                    if (NextChar() == 'f' && NextChar() == 'a' && NextChar() == 'u' && NextChar() == 'l' && NextChar() == 't' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordDefaultToken;
                    }
                }
                else if (ch == 'o')
                {
                    ...
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordDoToken;
                    }
                    if (NextChar() == 'u' && NextChar() == 'b' && NextChar() == 'l' && NextChar() == 'e' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordDoubleToken;
                    }
                }
            }
            else if (ch == 'c')
            {
                ...
                if (ch == 'o')
                {
                    ...
                    if (ch == 'n')
                    {
                        ...
                        if (ch == 't')
                        {
                            if (NextChar() == 'i' && NextChar() == 'n' && NextChar() == 'u' && NextChar() == 'e' && !IsNamePart(Peek()))
                            {
                                MarkTokenEnd();
                        ...
                        {
                            if (NextChar() == 't' && !IsNamePart(Peek()))
                            {
                                MarkTokenEnd();
                                return Tokens.KeywordConstToken;
                            }
                        }
                    }
                }
                ...
                else if (ch == 'a')
                {
                    ...
                    if (ch == 't')
                    {
                        if (NextChar() == 'c' && NextChar() == 'h' && !IsNamePart(Peek()))
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordCatchToken;
                        }
                    }
                    else if (ch == 's')
                    {
                        ...
                        {
                            MarkTokenEnd();
                            return Tokens.KeywordCaseToken;
                        }
                    }
                }
                else if (ch == 'h')
                {
                    if (NextChar() == 'a' && NextChar() == 'r' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordCharToken;
                    }
                }
            }
            else if (ch == 'b')
            {
                ...
                if (ch == 'y')
                {
                    if (NextChar() == 't' && NextChar() == 'e' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordByteToken;
                    }
                }
                else if (ch == 'r')
                {
                    if (NextChar() == 'e' && NextChar() == 'a' && NextChar() == 'k' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                ...
                {
                    if (NextChar() == 'o' && NextChar() == 'l' && NextChar() == 'e' && NextChar() == 'a' && NextChar() == 'n' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordBooleanToken;
                    }
                }
            }
            else if (ch == 'a')
            {
                ...
                if (ch == 'b')
                {
                    if (NextChar() == 's' && NextChar() == 't' && NextChar() == 'r' && NextChar() == 'a' && NextChar() == 'c' && NextChar() == 't' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordAbstractToken;
                    }
                }
                else if (ch == 's')
                {
            ...
            else if (ch == 'n')
            {
                ...
                if (ch == 'e')
                {
                    if (NextChar() == 'w' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordNewToken;
                    }
                }
                else if (ch == 'a')
                {
                    if (NextChar() == 't' && NextChar() == 'i' && NextChar() == 'v' && NextChar() == 'e' && !IsNamePart(Peek()))
                    {
                        MarkTokenEnd();
                        return Tokens.KeywordNativeToken;
                    }
                }
            }
            else if (ch == 'l')
            {
                if (NextChar() == 'o' && NextChar() == 'n' && NextChar() == 'g' && !IsNamePart(Peek()))
                {
                    MarkTokenEnd();
                    return Tokens.KeywordLongToken;
                }
            }
            if (ch == 'n')
            {
                if (NextChar() == 'u' && NextChar() == 'l' && NextChar() == 'l' && !IsNamePart(Peek()))
                {
                    MarkTokenEnd();
                    return Tokens.KeywordNullToken;
                }
            }
            else if (ch == 't')
            {
            ...
            while (true) {
                ...
                switch (ch) {
                    ...
                    default:
                        ...
                        // indentation level - Java says that blank lines are ignored.
        ...
        [Serializable]
        class IncompleteComment : IEquatable<IncompleteComment> {
            public readonly bool IsRaw, IsUnicode, IsTripleQuoted, IsSingleTickQuote;

            public IncompleteComment(bool isSingleTickQuote, bool isRaw, bool isUnicode, bool isTriple) {
                IsRaw = isRaw;
                IsUnicode = isUnicode;
            ...
            public override bool Equals(object obj) {
                IncompleteComment oth = obj as IncompleteComment;
                if (oth != null) {
                    return Equals(oth);
                }
                return false;
            ...
            public static bool operator ==(IncompleteComment left, IncompleteComment right) {
            ...
            public bool Equals(IncompleteComment other) {
                if (other == null) {
                    return false;
                }
        ...
        struct State : IEquatable<State> {
            ...
            public IncompleteComment IncompleteComment;
            ...
            public State(State state, bool verbatim) {
                ...
                IncompleteComment = state.IncompleteComment;
                if (verbatim) {
            ...
            public static bool operator ==(State left, State right) {
                ...
                return left.BraceLevel == right.BraceLevel &&
                       ...
                       left.IncompleteComment == right.IncompleteComment;
            }
        ...
        private void RefillBuffer() {
            ...
            try {
                ...
                _end += count;
                //System.Diagnostics.Debug.Write( new string(_buffer));
            } catch (BadSourceException bse) {
                StreamReader streamReader = _reader as StreamReader;
                if (streamReader != null && streamReader.CurrentEncoding != JavaAsciiEncoding.SourceEncoding) {
                    _errors.Add(
                        String.Format(
