...
extern int _VIsKnownTypeName(const char *aTypeName);
...
{
  // sets up lex to scan the data
  _VSetDataToScan(someData); 
...
void _VRegisterTypeName(const char *aTypeName)
{
  ...
  //[typeNameDictionary setObject:[NSString stringWithCString:aTypeName encoding:NSUTF8StringEncoding] forKey:[NSString stringWithCString:aTypeName encoding:NSUTF8StringEncoding]];
}

int _VIsKnownTypeName(const char *aTypeName)
{
  return 1;
  //return (nil != [typeNameDictionary objectForKey:[NSString stringWithCString:aTypeName encoding:NSUTF8StringEncoding]]);
}
